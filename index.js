...
app.post("/api/buy", async (req, res) => {
    ...
    if (secret && secret != "undefined") {
        var account =
            await sql`select skins, coins, username
                      from accounts
                      where secret = ${secret}`;
        if (account[0]) {
            acc = account[0];
            var yo =
                await sql`SELECT sum(coins)
                          FROM games
                          WHERE lower(name) = ${acc.username.toLowerCase()}
                            AND verified = 'true';`;
...
app.post("/api/changepassword", async (req, res) => {
    ...
    var account = await sql`SELECT password, secret
                            FROM accounts
                            WHERE secret = ${secret}`;
    if (!account[0]) {
    ...
    newSecret = uuid.v4();
    newAccount = await sql`UPDATE accounts
...
app.post("/api/changename", async (req, res) => {
    ...
    var account = await sql`select username
                            from accounts
                            where secret = ${secret}`;
    if (!account[0]) {
        res.status(400).status(400).send({error: "Invalid secret"});
    ...
    var daysSince = await sql`select (now()::date - lastusernamechange::date) as days
                              from accounts
                              where secret = ${secret}`;
    ...
    var account1 = await sql`select username
                             from accounts
                             where lower(username) = ${newUsername.toLowerCase()}`;
    if (account1[0]) {
    ...
    await sql`UPDATE games
              ...
              WHERE lower(name) = ${oldUsernameLower}
                AND verified = true`;
...
app.post("/api/signup", checkifMissingFields, async (req, res) => {
    ...
    async function doit() {
        bcrypt.hash(req.body.password, 10, (err, hash) => {
            ...
            var secret = uuid.v4();
            sql`insert into accounts(username, password, email, secret, skins, lastlogin)
                values (${username}, ${hash}, ${req.body.email}, ${secret},
                        ${JSON.stringify({collected: ["player"], selected: "player"})}, ${Date.now()})`;
        ...
        if (account[0]) {
            ...
            var yo =
                await sql`SELECT sum(coins)
                          ...
                          WHERE lower(name) = ${acc.username.toLowerCase()}
                            AND verified = 'true';`;
            var skinStats = await sql`SELECT skins - > 'collected' as collected
                                      from accounts;`;
            var counts = {};
            skinStats.forEach((x) => {
...
app.get("/leaderboard", async (req, res) => {
    ...
    if (type !== "xp" && !type.startsWith("total")) {
        ...
        if (duration != "all") {
            if (type == "xp") {
                var lb =
                    await sql`select name, (sum(coins) + (sum(kills) * 300)) as xp
                              from games
                              where verified = true
                              ...
                              group by name
                              order by xp desc limit 103`;
            } else {
                var lb =
                    await sql`select name, sum(${sql(type.slice(5))}) as ${sql(type.slice(5))}
                              from games
                              where verified = true
                                and EXTRACT(EPOCH FROM (now() - created_at)) < ${
                                      duration == "day" ? "86400" : "608400"
                              ...
                              group by name
                              order by ${sql(type.slice(5))} desc limit 103`;
            }
        } else {
            if (type == "xp") {
                var lb =
...
app.get("/:user", async (req, res, next) => {
    ...
    if (!dbuser[0]) {
        ...
        var yo =
            await sql`SELECT *
                      FROM games
                      WHERE lower(name) = ${user.toLowerCase()}
                        AND verified = 'true';`;
    ...
    */
        ...
        var stats = await sql`
            select a.dt, b.name, b.xp, b.kills, b.coins, b.time
            from (select distinct(created_at::date) as Dt
                  from games
                  where created_at >= ${
                          dbuser[0].created_at
                  }::date-1
                  order by created_at:: date) a
                     left join
                 (SELECT name,
                         created_at::date as dt1,(sum(coins) + (sum(kills) * 300)) as xp,
                         sum(kills) as kills,
                         sum(coins) as coins,
                         sum(time) as time
                  FROM games
                  WHERE verified='true' and lower (name)=${user.toLowerCase()}
                  group by name, created_at:: date) b on a.dt = b.dt1
            order by a.dt asc
        `;
        var lb =
            await sql`select name, (sum(coins) + (sum(kills) * 300)) as xp
...
io.on("connection", async (socket) => {
    ...
    socket.on("go", async ([r, captchatoken, tryverify, options]) => {
        console.log("go:", r, captchatoken, tryverify, options);
    ...
    socket.on("disconnect", () => {
        ...
        sql`INSERT INTO games (name, coins, kills, time, verified)
            VALUES (${thePlayer.name}, ${thePlayer.coins}, ${thePlayer.kills}, ${Date.now() - thePlayer.joinTime},
                    ${thePlayer.verified})`;
    ...
    for (var player of Object.values(PlayerList.players)) {
        if (player && !player.ai) {
            ...
            if (socket) {
                ...
                socket.disconnect();
                await sql`INSERT INTO games (name, coins, kills, time, verified)
                          VALUES (${
                                          player.name
