...
class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        grecaptcha.ready(() => {
            grecaptcha.execute(CAPTCHASITE, {action: "join"}).then((thetoken) => {
                ...
                //go packet
                var server = this.options.server == "eu1" ? "wss://europesword.herokuapp.com" : this.options.server == "us2" ? "wss://06039e4d-eccc-4c28-b230-52e9f704012c.id.repl.co/" : "wss://sword-io-game.herokuapp.com";
                ...
                this.socket.on("connected", () => {
                    ...
                    console.log("Connected to server!", this.options);
                    ...
                    this.socket.on("collected", ([coinId, playerId, coin]) => {
                        ...
                        if (this.coins.find(coin => coin.id == coinId)) this.coins.find(coin => coin.id == coinId).state = {
                            collected: true,
    ...
    update(time, delta) {
        ...
        */
        ...
        var enemies = this.all.players.map((p) => {
            return {playerObj: p};
        });
